...
!!! tip "trime新版本中出现了新机制"
    新版本的trime新增俩初始化二选一的配置，一个是外部目录和data内部目录同时使用，在部署的时候将外部目录覆盖内部文件，还有一个是只采用data目录。
    那么前者从运行逻辑上会覆盖data生成的类似wanxiang.custom.yaml文件，导致后面的指令切换输入类型无法生效。
    需要在外部文件中手动修改custom，相关方法参考手动修改的相关说明，而后者使用内部目录则可以使用小企鹅类似的方式导入文件或者采用万象更新app导入方案文件，只有内部的时候指令依旧可用。
